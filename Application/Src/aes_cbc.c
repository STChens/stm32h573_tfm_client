...
int aes_cbc_test(void)
{
  ...
  LOG_MSG("Start %s (PSA_ALG_CBC_NO_PADDING) ...\r\n", __FUNCTION__);

  /* --------------------------------------------------------------------------
   * Create the PSA key
   * --------------------------------------------------------------------------
  */
  //LOG_MSG("Create the PSA key\r\n");
  //LOG_MSG("Init the key attributes\r\n");
  /* Init the key attributes */
  key_attributes = psa_key_attributes_init();

  ////LOG_MSG("Setup the key policy\r\n");
  ...
  //LOG_MSG("Reset the key attribute\r\n");
  ...
  */
  ...
  LOG_MSG("\t1) Test SINGLE CALL USAGE\r\n");
  ...
  //LOG_MSG("Verify API returned value psa_cipher_encrypt\r\n");
  ...
  //LOG_MSG("Verify generated data size is the expected one\r\n");
  ...
  //LOG_MSG("Verify API returned value psa_cipher_decrypt\r\n");
  /* Verify API returned value */
  if (retval != PSA_SUCCESS)
  {
    Error_Handler();
  ...
  //LOG_MSG("Verify generated data are the expected ones\r\n");
  /* Verify generated data are the expected ones */
  if (memcmp(Plaintext, Computed_Plaintext, computed_size) != 0)
  {
    Error_Handler();
  }
  LOG_MSG("\t...Test Result OK.\r\n", __FUNCTION__);
  ...
  LOG_MSG("\t2) MULTIPLE CALLS USAGE\r\n");
  operation = psa_cipher_operation_init();

  //LOG_MSG("Setup the context for encrypt operation\r\n");
  ...
  //LOG_MSG("Setup of the Initialization Vector (IV) into the context\r\n");
  ...
  //LOG_MSG("Process with encryption of the plaintext by chunk of CHUNK_SIZE bytes\r\n");
  ...
  for (index = 0; index < (sizeof(Plaintext) - CHUNK_SIZE); index += CHUNK_SIZE)
  {
    ...
    //LOG_MSG("Verify API returned value psa_cipher_update\r\n");
    /* Verify API returned value */
    if (retval != PSA_SUCCESS)
    {
      Error_Handler();
  ...
  //LOG_MSG("Process with encryption of the last part if needed\r\n");
  ...
  //LOG_MSG("Finish properly the encrypt\r\n");
